...
public class SlidingDrawer extends ViewGroup
{
	...
	public SlidingDrawer(final Context context, final AttributeSet attributeSet, final int defStyle)
	{
		...
		final TypedArray typedArray = context.obtainStyledAttributes(attributeSet, R.styleable.SlidingDrawer, defStyle, 0);
		...
		final int orientation = typedArray.getInt(R.styleable.SlidingDrawer_orientation, ORIENTATION_VERTICAL);
		...
		if (handleResId == Integer.MIN_VALUE) { throw new IllegalArgumentException("The handle attribute is required and must refer to a valid child."); }

		contentResId = typedArray.getResourceId(R.styleable.SlidingDrawer_content, Integer.MIN_VALUE);
		if (contentResId == Integer.MIN_VALUE) { throw new IllegalArgumentException("The content attribute is required and must refer to a valid child."); }

		if (handleResId == contentResId) { throw new IllegalArgumentException("The content and handle attributes must refer to different children."); }
	...
	protected void onFinishInflate()
	{
		...
		if (handleView == null) { throw new IllegalArgumentException("The handle attribute is must refer to an existing child."); }

		contentView = findViewById(contentResId);
		if (contentView == null) { throw new IllegalArgumentException("The content attribute is must refer to an existing child."); }
	...
	protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec)
	{
		...
		if (widthSpecMode == MeasureSpec.UNSPECIFIED || heightSpecMode == MeasureSpec.UNSPECIFIED) { throw new IllegalStateException("SlidingDrawer cannot have unspecified dimensions"); }

		measureChild(handleView, widthMeasureSpec, heightMeasureSpec);

		topOffset = -handleView.getMeasuredHeight();

		if (isVertical)
		{
			final int height = heightSpecSize - handleView.getMeasuredHeight() - topOffset;

			contentView.measure(MeasureSpec.makeMeasureSpec(widthSpecSize, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(height, MeasureSpec.EXACTLY));

		} else
		{
			// final int width = widthSpecSize - handleView.getMeasuredWidth() -
			// topOffset;
			final int width = widthSpecSize;

			contentView.measure(MeasureSpec.makeMeasureSpec(width, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(heightSpecSize, MeasureSpec.EXACTLY));
		}
	...
	protected void onLayout(final boolean changed, final int left, final int top, final int right, final int bottom)
	{
		...
		if (!isTracking)
		{
			...
			if (isVertical)
			{
				...
				contentView.layout(0, topOffset + handleMeasuredHeight, contentView.getMeasuredWidth(), topOffset + handleMeasuredHeight + contentView.getMeasuredHeight());
			...
			} else
			{
				...
				contentView.layout(topOffset + handleMeasuredWidth, 0, topOffset + handleMeasuredWidth + contentView.getMeasuredWidth(), contentView.getMeasuredHeight());
			}

			handleView.layout(handleLeft, handleTop, handleLeft + handleMeasuredWidth, handleTop + handleMeasuredHeight);
	...
	protected void dispatchDraw(final Canvas canvas)
	{
		...
		if (isTracking || isAnimating)
		{
			...
			if (bitmap == null)
			{
				...
				canvas.translate(isVertical ? 0 : handleView.getLeft() - topOffset, isVertical ? handleView.getTop() - topOffset : 0);
	...
	public boolean onInterceptTouchEvent(final MotionEvent event)
	{
		...
		if (!isTracking && !frameRect.contains((int) x, (int) y)) { return false; }
		...
		if (isTracking)
		{
			...
			case MotionEvent.ACTION_CANCEL:
				...
				if (Math.abs(velocity) < maximumTapVelocity)
				{
					...
					if (isVertical ? (isExpanded && top < tapThreshold + topOffset) || (!isExpanded && top > bottomOffset + getBottom() - getTop() - handleHeight - tapThreshold)
							: (isExpanded && left < tapThreshold + topOffset) || (!isExpanded && left > bottomOffset + getRight() - getLeft() - handleWidth - tapThreshold))
					{
		...
		if (!isAnimating)
		{
			...
			if (contentView.isLayoutRequested())
			{
				...
				if (isVertical)
				{
					...
					contentView.measure(MeasureSpec.makeMeasureSpec(getRight() - getLeft(), MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(height, MeasureSpec.EXACTLY));

					contentView.layout(0, topOffset + handleHeight, contentView.getMeasuredWidth(), topOffset + handleHeight + contentView.getMeasuredHeight());
				...
				} else
				{
					...
					contentView.measure(MeasureSpec.makeMeasureSpec(width, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(getBottom() - getTop(), MeasureSpec.EXACTLY));

					contentView.layout(handleWidth + topOffset, 0, topOffset + handleWidth + contentView.getMeasuredWidth(), contentView.getMeasuredHeight());
				}
			}
	...
	private void prepareTracking(final int position)
	{
		...
		} else
		{
			...
			animationPosition = bottomOffset + (isVertical ? getHeight() - handleHeight : getWidth() - handleWidth);
	...
	private void incrementAnimation()
	{
		...
		animationPosition = animationPosition + (animatedVelocity * time) + (0.5f * animatedAcceleration * time * time);
	...
	private void doAnimation()
	{
		...
		if (isAnimating)
		{
			...
			} else
			{
				...
				drawerHandler.sendMessageAtTime(drawerHandler.obtainMessage(MESSAGE_ANIMATE), currentAnimationTime);
			}
		}
	}
	...
	private void performFling(final int position, final float velocity, final boolean always)
	{
		...
		if (isExpanded)
		{
			...
			if (always || (velocity > maximumMajorVelocity || (position > topOffset + (isVertical ? handleHeight : handleWidth) && velocity > -maximumMajorVelocity)))
			{
		...
		} else
		{
			...
			if (!always && (velocity > maximumMajorVelocity || (position > (isVertical ? getHeight() : getWidth()) / 2 && velocity > -maximumMajorVelocity)))
			{
	...
	private void moveHandle(final int position)
	{
		...
		if (isVertical)
		{
			...
			{
				handleView.offsetTopAndBottom(bottomOffset + getBottom() - getTop() - handleHeight - handleView.getTop());
			...
			} else
			{
				...
				invalidateRect.union(frameRect.left, frameRect.top - deltaY, frameRect.right, frameRect.bottom - deltaY);

				invalidateRect.union(0, frameRect.bottom - deltaY, getWidth(), frameRect.bottom - deltaY + contentView.getHeight());
		...
		} else
		{
			...
			{
				handleView.offsetLeftAndRight(bottomOffset + getRight() - getLeft() - handleWidth - handleView.getLeft());
			...
			} else
			{
				...
				invalidateRect.set(frameRect);
				invalidateRect.union(frameRect.left - deltaX, frameRect.top, frameRect.right - deltaX, frameRect.bottom);

				invalidateRect.union(frameRect.right - deltaX, 0, frameRect.right - deltaX + contentView.getWidth(), getHeight());
